...
import { POSE_CONFIG } from './pose.js';
...
function initSettings() {
  ...
  restoreDefaultsBtn.addEventListener('click', (e) => {
    ...
    document.getElementById('gif-colors-val').value = '256';
    document.getElementById('gif-diff-thresh').value = '32';
    document.getElementById('gif-diff-thresh-val').value = '32';
    document.getElementById('gif-global-palette').checked = false;
    ...
    localStorage.removeItem('gifColors');
    localStorage.removeItem('gifDiffThresh');
    localStorage.removeItem('gifGlobalPalette');
...
function setupThemeToggle() {
  ...
  function updateLightClass() {
    const explicit = root.getAttribute('data-theme');
    const isLight = explicit === 'light' || (!explicit && !darkMq.matches);
    root.classList.toggle('light-theme', isLight);
  }

  toggle.addEventListener('click', () => {
    const current = root.getAttribute('data-theme');
    const newTheme = current === 'light' ? 'dark' : current === 'dark' ? 'light' : (darkMq.matches ? 'light' : 'dark');
    root.setAttribute('data-theme', newTheme);
    localStorage.setItem('theme', newTheme);
    updateLightClass();
  });

  darkMq.addEventListener('change', updateLightClass);
}
