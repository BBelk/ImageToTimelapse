...
import { POSE_CONFIG, getDisplayRect } from './pose.js';
...
const gifColorsVal = document.getElementById('gif-colors-val');
const gifDiffThreshSlider = document.getElementById('gif-diff-thresh');
const gifDiffThreshVal = document.getElementById('gif-diff-thresh-val');
const gifGlobalPaletteToggle = document.getElementById('gif-global-palette');
...
async function generateGif(validCmps, w, h, loopGif, useTransitions, tType, tDur, durFirst, durMiddle, durLast) {
  ...
  const paletteSize = parseInt(gifColorsSlider.value) || 256;
  const DIFF_THRESHOLD = parseInt(gifDiffThreshSlider.value) || 32;
  ...
  const USE_GLOBAL_PALETTE = gifGlobalPaletteToggle.checked;
  const TRANSPARENT_INDEX = 0;

  dlog('info', 'GIF encoding started', { paletteSize, globalPalette: USE_GLOBAL_PALETTE, diffThreshold: DIFF_THRESHOLD, lossy: LOSSY_ROUND, frameDiff: USE_FRAME_DIFF });

  let globalPalette = null;
  if (USE_GLOBAL_PALETTE) {
    showProgress('Building global palette...', 5);
    ...
    const globalPaletteSize = USE_FRAME_DIFF ? Math.min(paletteSize, 255) : Math.min(paletteSize, 256);
    const basePalette = quantize(allSamples, globalPaletteSize);
    globalPalette = USE_FRAME_DIFF ? [[0, 0, 0], ...basePalette] : basePalette;
    dlog('info', 'Global palette built', { colors: globalPalette.length });
  } else {
  ...
  function encodeFrame(canvas, delayMs) {
    ...
    if (USE_GLOBAL_PALETTE) {
      ...
      const needsTransparent = USE_FRAME_DIFF && !isFirstFrame;
      const actualPaletteSize = needsTransparent ? Math.min(paletteSize, 255) : Math.min(paletteSize, 256);
      const basePalette = quantize(data, actualPaletteSize);
      framePalette = needsTransparent ? [[0, 0, 0], ...basePalette] : basePalette;
    }
...
export function setupOutput() {
  ...
  const _savedGifColors = localStorage.getItem('gifColors');
  if (_savedGifColors) {
    gifColorsSlider.value = _savedGifColors;
    gifColorsVal.value = _savedGifColors;
  }
  gifColorsSlider.addEventListener('input', () => {
    gifColorsVal.value = gifColorsSlider.value;
    localStorage.setItem('gifColors', gifColorsSlider.value);
  });
  gifColorsVal.addEventListener('input', () => {
    const val = Math.max(8, Math.min(256, parseInt(gifColorsVal.value) || 256));
    gifColorsVal.value = val;
    gifColorsSlider.value = val;
    localStorage.setItem('gifColors', val);
  });

  const _savedGifDiffThresh = localStorage.getItem('gifDiffThresh');
  if (_savedGifDiffThresh) {
    gifDiffThreshSlider.value = _savedGifDiffThresh;
    gifDiffThreshVal.value = _savedGifDiffThresh;
  }
  gifDiffThreshSlider.addEventListener('input', () => {
    gifDiffThreshVal.value = gifDiffThreshSlider.value;
    localStorage.setItem('gifDiffThresh', gifDiffThreshSlider.value);
  });
  gifDiffThreshVal.addEventListener('input', () => {
    const val = Math.max(0, Math.min(128, parseInt(gifDiffThreshVal.value) || 32));
    gifDiffThreshVal.value = val;
    gifDiffThreshSlider.value = val;
    localStorage.setItem('gifDiffThresh', val);
  });

  if (localStorage.getItem('gifGlobalPalette') !== null) {
    gifGlobalPaletteToggle.checked = localStorage.getItem('gifGlobalPalette') === 'true';
  }
  gifGlobalPaletteToggle.addEventListener('change', () => localStorage.setItem('gifGlobalPalette', gifGlobalPaletteToggle.checked));
